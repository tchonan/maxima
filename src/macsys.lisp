...
(defun $system (&rest args)
  ...
  (let ((s (and (boundp '*socket-connection*) *socket-connection*))
    ...
    #+sbcl (sb-ext:run-program shell (cons shell-opt (mapcar '$sconcat args)) :search t :output (or s t))
    #+openmcl (ccl::run-program shell (cons shell-opt (mapcar '$sconcat args)) :output (or s t))
